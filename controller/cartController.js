...
const getUserCartInfo =  asyncHandler(async (req, res) => {
    ...
    if(oldData){   
        let data =  await Product.find({ 'product__id': { $in: oldData._doc.product__ides } }); 
            ...
            res.status(200).send({message: 'Successfully cart data loaded', data, status__code: 200})
    }else{ 
       ...
       res.status(200).send({message: 'Successfully   cart  data loaded', data: emptyUserCart, status__code: 200})
    }
})
...
const cartProductQuantityIncrease =  asyncHandler(async (req, res) => {
    let data = req.body; 
    let {userKey, productId} = data;
    try {
        ...
        let dProductII = userCartDatabase.product__ides.indexOf(productId); 
            ...
            userCartDatabase.product__id__with__quantity[dProductII].quantity = userCartDatabase.product__id__with__quantity[dProductII].quantity + 1;
            let upData = await AddToCartModel.updateOne({_id: userCartDatabase._id},{product__ides: userCartDatabase.product__ides, product__id__with__quantity: userCartDatabase.product__id__with__quantity})
                if(upData){
                    let uData =  await Product.find({ 'product__id': { $in: userCartDatabase.product__ides } }); 
                    uData = await filterUtils.uniqueProductGenerator(uData);
                    uData = await filterUtils.GetProductQuantityProductAllPriceAndMany(uData, userCartDatabase);
                    res.status(200).send({message: 'Successfully product count increased', data: uData, status__code: 200});
                }else{
                    res.status(400);
                    throw new Error("There was a server side error")
                }
...
const cartProductQuantityDecrease =  asyncHandler(async (req, res) => {
    let data = req.body; 
    let {userKey, productId} = data;
    try {
        ...
        let dProductII = userCartDatabase.product__ides.indexOf(productId); 
            ...
            let upData = await AddToCartModel.updateOne({_id: userCartDatabase._id},{product__ides: userCartDatabase.product__ides, product__id__with__quantity: userCartDatabase.product__id__with__quantity})
                if(upData){
                    ...
                    uData = await filterUtils.uniqueProductGenerator(uData);
                    uData = await filterUtils.GetProductQuantityProductAllPriceAndMany(uData, userCartDatabase);
                    res.status(200).send({message: 'Successfully product count Decrease', data: uData, status__code: 200});
                }else{
                    res.status(400);
                    throw new Error("There was a server side error")
                }
...
module.exports = {addToCart, removeFromCart, getUserCartInfo, cartProductQuantityIncrease, cartProductQuantityDecrease};
...
const Recommended = () => {
    ...
    useEffect(()=>{
        axios.get(serverPort+'/api/promotion/singlePromotionGet',{headers:{promotion: 'recommended'.replace(/__/g,'underScore')}})
        .then(res => { 
    ...
    return (
        <div>
            {
                products.length ? <UpgradeYourGamingStation product={{products, promotion__name, name: 'Recommended'}}/> : ''
            }
        </div>
    );
};

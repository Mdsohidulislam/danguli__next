...
const InnovatedGadget = () => {
    ...
    let promotion__name = 'innovated__gadget';
    useEffect(()=>{
        axios.get(serverPort+'/api/promotion/single__promotion__get',{headers:{promotion__name: 'innovated__gadget'.replace(/__/g,'underScore')}})
        .then(res => { 
    ...
    return (
        <div>
            {
                products.length ? <UpgradeYourGamingStation product={{products, promotion__name, name: 'Innovated Gadget'}}/> : ''
            } 
        </div>
    );
};

export default InnovatedGadget;
...
const PreOrder = () => {
    ...
    useEffect(()=>{
        axios.get(serverPort+'/api/promotion/singlePromotionGet',{headers:{promotion: 'pre__order'.replace(/__/g,'underScore')}})
        .then(res => { 
    ...
    return (
        <div>
            {
                products.length ? <UpgradeYourGamingStation product={{products, promotion__name, name: 'Pre-order'}}/> : ''
            }
        </div>
    );
};
